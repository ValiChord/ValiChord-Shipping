...
| [`docs/19-handover-brief.md`](docs/19-handover-brief.md) | *Superseded by [`docs/26`](docs/26-handover-find-a-holochain-project.md).* The stopping condition that was missing, four corrections to carry, and what not to do |
...
| [`docs/24-fueleu-architecture.md`](docs/24-fueleu-architecture.md) | **The first candidate where peers can verify rather than merely witness** — a FuelEU compliance balance is arithmetic, not judgement. One DNA cloned per vessel because the liability attaches to the hull, and countersigning for the charterer handover |
| [`docs/25-seven-tests.md`](docs/25-seven-tests.md) | **⭐ START HERE if you are evaluating a verification idea in any industry.** The seven tests that closed all seven shipping candidates, ordered cheapest first — plus the five findings that survived them |
| [`docs/26-handover-find-a-holochain-project.md`](docs/26-handover-find-a-holochain-project.md) | **⭐ PASTE THIS INTO A FRESH SESSION.** The current task — find a shipping problem Holochain genuinely fits, not a ValiChord one. Ten ruled-out candidates, verified Holochain facts, where to search, and the drift to avoid repeating. Supersedes `docs/19` |
